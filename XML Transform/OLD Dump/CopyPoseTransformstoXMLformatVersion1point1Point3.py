bl_info = {
    ...
    "description": "Copy and Paste pose bone local location to and from XML format (flip Z only)",
    "category": "Animation",
}
...
class CopyBoneTransformOperator(bpy.types.Operator):
    ...
    bl_description = "Copy the selected pose bone's local location as formatted XML (Z flipped)"
...
class PasteBoneTransformOperator(bpy.types.Operator):
    ...
    bl_description = "Reads XML from clipboard and applies local location to selected pose bone (Z flipped)"
    ...
    def execute(self, context):
        ...
        bone.location = (x, y, -z)  # Z flip only
