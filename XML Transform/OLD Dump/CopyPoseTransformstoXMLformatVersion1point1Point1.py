bl_info = {
    ...
    "description": "Copy and Paste pose bone local location to and from XML format (Breakpoint-compatible)",
    "category": "Animation",
}
...
class CopyBoneTransformOperator(bpy.types.Operator):
    ...
    bl_description = "Copy the selected pose bone's local location as formatted XML (with Z flip)"
    ...
    def execute(self, context):
        ...
        xml = (
            f"<X>{loc.x}</X>\n"
            f"\t\t\t\t<Y>{-loc.z}</Y>\n"  # <- Flipping Z when copying
            f"\t\t\t\t<Z>{-loc.y}</Z>"
        )

        context.window_manager.clipboard = xml
        self.report({'INFO'}, "Copied pose bone location to clipboard (Z flipped)")
        return {'FINISHED'}
...
class PasteBoneTransformOperator(bpy.types.Operator):
    ...
    bl_description = "Reads XML from clipboard and applies local location to selected pose bone (with Z flip)"
    ...
    def execute(self, context):
        ...
        bone.location = (x, -z, -y)  # <- Flip Y and Z back
        self.report({'INFO'}, "Applied XML location to bone (Z flipped)")
        return {'FINISHED'}
